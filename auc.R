...
res <- file(filepath, "w")
write("Results per language and r per language set for all trainfiles
...
fme <- file(fpath, "w")
write("Means and standard deviations per r and language per language set over all runs\n", fme)
...
write("\n\n---------------------------------------------", res)
write("LITERAL TEST ALPHABET", res) 
write("---------------------------------------------\n", res)
globalmeans <- list()
globalstds <- list()
for(lang in langs) {
  ...
  for(r in 3:5) {
    ...
    for (tre in 0:2) {
      aucs <- list()
      for (re in 0:2) {
        score = readLines(sprintf("eo_%s_test_t%s-%s.txt", r, tre, re))
        data_e = as.data.frame(score)
        data_e$label <- 0
        score = readLines(sprintf("%s_%s_test_t%s-%s.txt", lang, r, tre, re))
        data_t = as.data.frame(score)
        data_t$label <- 1
        data_t = rbind(data_e, data_t)
        auc = auc(roc(data_t$score, factor(data_t$label)))
...
#print(globalmeans)
write("\nLITERAL TEST ALPHABET", fme) 
writemeans(globalmeans, globalstds, langs)


write("\n\n---------------------------------------------", res)
write("TRANSLITERAL TEST ALPHABET", res) 
write("---------------------------------------------\n", res)
globalmeans <- list()
globalstds <- list()
for(lang in tlangs) {
    ...
    for (tre in 0:2) {
      ...
      for (re in 0:2) {
        result <- tryCatch({
          ...
          data_e = as.data.frame(score)
          data_e$label <- 0
          score = readLines(sprintf("%s_%s_transl_test_t%s-%s.txt", lang, r, tre, re))
          data_t = as.data.frame(score)
          data_t$label <- 1
          data_t = rbind(data_e, data_t)
          auc = auc(roc(data_t$score, factor(data_t$label)))
...
#print(globalmeans)
write("\nTRANSLITERAL TEST ALPHABET", fme) 
writemeans(globalmeans, globalstds, tlangs)
...
#phonetic text, global alphabet for variant 0 ligature false
write("\n\n---------------------------------------------", res)
write("PHONETIC TEST ALPHABET", res) 
write("---------------------------------------------\n", res)
globalmeans <- list()
globalstds <- list()
for(lang in plangs) {
  ...
  for(r in 3:5) {
    ...
    for (tre in 0:1) {
      ...
      for (re in 0:1) {
        try({
          score = readLines(sprintf("eo_%s_ipa_test_t%s-%s.txt", r, tre, re))
          data_e = as.data.frame(score)
          data_e$label <- 0
          score = readLines(sprintf("%s_%s_ipa_test_t%s-%s.txt", lang, r, tre, re))
          data_t = as.data.frame(score)
          data_t$label <- 1
          data_t = rbind(data_e, data_t)
          auc = auc(roc(data_t$score, factor(data_t$label)))
